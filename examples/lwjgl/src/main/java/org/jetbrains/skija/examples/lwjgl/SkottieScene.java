...
public class SkottieScene extends Scene {
    ...
    @SneakyThrows
    public SkottieScene() {
        _variants = Files.list(Path.of("animations")).map(Path::getFileName).map(Path::toString).sorted().toArray(String[]::new);
    }
        ...
        if (animationVariant != _variants[_variantIdx]) {
            ...
            try {
                animation = new AnimationBuilder(AnimationBuilderFlag.DEFER_IMAGE_LOADING, AnimationBuilderFlag.PREFER_EMBEDDED_FONTS)
                        .setLogger(logger)
                        .buildFromFile("animations/" + _variants[_variantIdx]);
