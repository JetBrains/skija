...
public class ShapingScene implements Scene {
    ...
    public void draw(Canvas canvas, int width, int height, float dpi, int xpos, int ypos) {
        ...
        try (var shaper = Shaper.makeShapeThenWrap();
             var handler = new TextBlobHandler();
             var blob = shape(shaper, "hello мир дружба fi fl 👃 one two ثلاثة 12 👂 34 خمسة", lato36, FontMgr.getDefault(), null, true, width - 40, handler);)
        {
            ...
            for (var pair: handler._infos) {
                ...
                try (var builder = new TextBlobBuilder(); ) {
                    ...
                    // build details blob
                    builder.appendRun(inter11, "font " + info.getFont().getTypeface().getFamilyName(), 0, yPos);
